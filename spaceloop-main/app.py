...
from security import (
    ...
    login_required, roles_required, seeker_required, host_required, admin_required,
    ensure_booking_access, ensure_space_access, get_current_user
)
...
def create_app(test_config=None):
    ...
    # Authentication & User session injector
    @app.context_processor
    def inject_auth_context():
        user = get_current_user()
        current_persona = user.role if user else session.get('persona', 'seeker')
        return dict(
    ...
    def index():
        ...
        # If search or category query passed, render the marketplace results explorer
        if q or (category and category.lower() != 'all'):
    ...
    def login_page():
        user = get_current_user()
        if user:
            return redirect(url_for('host_dashboard' if user.role == 'owner' else 'seeker_dashboard'))
    ...
    def login_action():
        ...
        if not result.success:
            return render_template('login.html', error=result.error), 400

        session.clear()
        session['user_id'] = result.user['id']
        session['persona'] = result.user['role']

        if next_url and next_url.startswith('/') and not next_url.startswith('//'):
            return redirect(next_url)
        return redirect(url_for('host_dashboard' if result.user['role'] == 'owner' else 'seeker_dashboard'))
    ...
    def register_action():
        ...
        result = AuthService.register(name, email, password, role)
        if not result.success:
            return render_template('register.html', error=result.error), 400
        ...
        if role == 'owner':
            return redirect(url_for('verify_host_page'))
        return redirect(url_for('verify_student_page'))
    ...
    def printable_qr(space_id):
        ...
        space = Space.query.get_or_404(space_id)
        user = get_current_user()
        if not ensure_space_access(space, user):
            abort(403)
    ...
    def session_console(booking_id):
        ...
        user = get_current_user()
        if not ensure_booking_access(booking, user):
            abort(403)
        space = booking.space
    ...
    def api_verify_host():
        ...
        # Update authenticated host profile
        user = get_current_user()
        if user:
    ...
    def api_check_in(booking_id):
        ...
        user = get_current_user()
        if not ensure_booking_access(booking, user):
            return jsonify({'error': 'Forbidden', 'message': 'You are not authorized for this booking session.'}), 403
    ...
    def api_auth_register():
        ...
        session['user_id'] = result.user['id']
        session['persona'] = result.user['role']
        return jsonify({'success': True, 'user': result.user}), 201
    ...
    def api_auth_me():
        ...
        user = get_current_user()
        if not user:
            return jsonify({'error': 'Unauthorized', 'message': 'No active authenticated session'}), 401
        return jsonify({'user': user.to_dict()})

    @app.route('/api/auth/demo-login', methods=['POST'])
    def api_auth_demo_login():
        """Quick 1-click authenticated demo login for evaluators/testing."""
        data = request.get_json() or {}
        role = data.get('role', 'seeker')
        demo_user = AuthService.get_demo_user(role)
        if not demo_user:
            return jsonify({'error': 'Demo user not found'}), 404
        session.clear()
        session['user_id'] = demo_user.id
        session['persona'] = demo_user.role
        return jsonify({
            'success': True,
            'user': demo_user.to_dict(),
            'redirect': '/dashboard/host' if demo_user.role == 'owner' else '/dashboard/seeker'
        })
    ...
    def api_switch_persona():
        ...
        user = AuthService.get_demo_user(target_role)

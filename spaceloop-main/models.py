...
class User(db.Model):
    ...
    role = db.Column(db.String(20), nullable=False, default='seeker')  # 'seeker', 'owner', 'admin'
    ...
    def set_password(self, password: str):
        """Hashes and sets user password using PBKDF2:SHA256."""
        self.password_hash = generate_password_hash(password, method='pbkdf2:sha256')

    def check_password(self, password: str) -> bool:
        """Verifies candidate password against stored hash."""
        if not self.password_hash or self.password_hash == "sha256_mock_hash":
            return False

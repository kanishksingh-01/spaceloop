...
from chatbot import chat_reply, get_partner_tip
...
def create_app():
    ...
    def chat():
        ...
        # Privacy-safe partner notification: a derived care tip, never the raw
        # message she typed. This keeps the chat a genuinely private space while
        # still letting a connected partner know how to help.
        user = User.query.get(user_id)
        if user and user.partner_id:
            tip = get_partner_tip(user_text, result["mode"])
            if tip:
                db.session.add(Notification(recipient_id=user.partner_id, message=tip))
